...
def get_data():
    ...
    uum = np.load('data_gen/data_uvm.npy').astype(np.float32)
    uum_1d= np.load('data_gen/uv_1d.npy').astype(np.float32)
    uum = kzs * uum
    ...
    dcPs_s = np.load('data_gen/data_dcp23.npy').astype(np.float32)[[0, 1, 3, 4]].reshape((-1,  23))
    dcPs_s_test = np.load('data_gen/data_dcp23.npy').astype(np.float32)[[2]].reshape((-1,  23))
    dkxs_s = np.load('data_gen/data_dkx86.npy').astype(np.float32)[[0, 1, 3, 4]].reshape((-1, 86))
    dkxs_s_test = np.load('data_gen/data_dkx86.npy').astype(np.float32)[[2]].reshape((-1, 86))
    ...
    coodinates = np.load('data_gen/3d_coordinates.npy').reshape((5, -1, 3)).astype(np.float32)
    #正则化
    for i in range(5):
    ...
    coodinates_zz = coodinates[[0, 1, 3, 4]]
    coodinates_zz_test = coodinates[[2]]
